...
void insereServidor(MYSQL *conexao);
void insereServidor(MYSQL *conexao, MYSQL_RES *resp, MYSQL_ROW linhas);
			...
			if(userType==1){
				do{
					...
					switch(oper){
						...
						break;
						case 5:
							insereServidor(&conexao);
						break;
						case 6:
							retiraServidor(&conexao, resp, linhas);
						break;
						case 7:
...
void insereServidor(MYSQL *conexao){
	lp();
	printf("===========Insercao de Servidor===========\n");
	char query[TAMQUERY], nome[TAMNOMES], telefone[TAMTELEFONE], numServidor[TAMMATRICULA], senha[TAMSENHA], dataNascimento[TAMDATA];
	...
	printf("Informe o numero do servidor:");
	scanf("%[^\n]s", numServidor);
	getchar();
	while(strlen(numServidor)>TAMMATRICULA-1){
		lp();
		printf("Tamanho excede o máximo aceitavel! %d Caracteres\n", TAMMATRICULA-1);
		printf("Informe o numero do servidor:");
		scanf("%[^\n]s", numServidor);
	...
	printf("Informe a senha para o servidor:");
	scanf("%[^\n]s", senha);
	getchar();
	while(strlen(senha)>TAMSENHA-1){
		lp();
	...
	printf("Informe o telefone do servidor:");
	scanf("%[^\n]s", telefone);
	getchar();
	while(strlen(telefone)>TAMTELEFONE-1){
		lp();
		printf("Tamanho excede o máximo aceitavel! %d Caracteres\n", TAMTELEFONE-1);
		printf("Informe a matricula do servidor:");
		scanf("%[^\n]s", telefone);
		getchar();
	}
	printf("Informe a data de nascimento do servidor:");
	scanf("%[^\n]s", dataNascimento);
	getchar();
	while(strlen(dataNascimento)>TAMDATA-1){
		lp();
	...
	sprintf(query, "INSERT INTO Servidor(nome, senha, telefone, dataNascimento, numLivros, numServidor) VALUES('%s', '%s', '%s', '%s', 0, '%s');", nome, senha, telefone, dataNascimento, matricula);
	if(mysql_query(conexao, query)){
		lp();
		printf("Erro ao inserir o servidor!\n");
		printf("%s\n", mysql_error(conexao));
		printf("Erro numero %d\n", mysql_errno(conexao));
	}
	else{
		printf("Servidor inserido com sucesso!\n");
	}
	create table Servidor(id_servidor primary key auto_increment, nome varchar(50)not null, senha varchar(14) not null, telefone varchar(12), dataNascimento date not null, numLivros int not null, numServidor int not null unique);	
}

void retirarServidor(MYSQL *conexao, MYSQL_RES *res, MYSQL_ROW linhas){
	char query[TAMQUERY], numServidor[TAMMATRICULA], teste[4];
	int cont;
	printf("Informe o numero do servidor a ser retirado:");
	scanf("%s", numServidor);
	while(strlen(numServidor)>TAMMATRICULA-1){
		lp();
		printf("Tamanho excede o máximo aceitavel! %d Caracteres\n", TAMDATA-1);
	...
	sprintf(query, "SELECT numLivros FROM Servidor WHERE numServidor=%s;", numServidor);
	if(mysql_query(conexao, query)){
		lp();
		printf("Erro ao conectar a tabela de Servidor! %s", mysql_error(conexao));
	}
	else{
		resp=mysql_store_result(conexao);
		if(!resp){
			...
			if((linhas=mysql_fetch_row(resp))==0){
				lp();
				printf("Este numero nao esta relacionado a nenhum servidor cadastrado\n\n");
			}
			else{
				if(strcmp(linhas[0], "0")==0){
					printf("Tem certeza que quer retirar o servidor?[SIM/NAO]");
					scanf("%s", teste);
					getchar();
					for(cont=0; cont<strlen(teste); cont++){
						teste[cont]=tolower(teste[cont]);
					...
					if(strcmp(teste, "sim")==0){
						sprintf(query, "DELETE FROM Servidor WHERE numServidor=%s;", numServidor);
						if(mysql_query(conexao, query)){
							lp();
							printf("Nao foi possivel retirar o servidor! %s\n\n!", mysql_error(conexao));
						}
						else{
							lp();
							printf("Servidor rerirado com sucesso!\n\n");
						}
					}
					else{
						lp();
					...
					lp();
					printf("Nao foi possivel retirar o Servidor! Este possui livros emprestado!\n\n");
				}
			}
		}
	}
...
void devolverLivro(MYSQL *conexao, MYSQL_RES *resp, MYSQL_ROW linhas){
	...
	printf("Informe o ID do livro a ser retirado:");
	scanf("%d", &idLivro);
	sprintf(query, "SELECT alugado" );
